...
/**
 * Command-line application for listing the members of a Grouper group.
 *
 * This application outputs the names (equivalent to the "umDisplayName"
 * field in LDAP) of the members of the specified Grouper group.
 */
public class GrouperListCli {
  public static final Logger log = LoggerFactory.getLogger(GrouperListCli.class);

  /**
   * Called by the "grouper-list" script (configured in th
  ...
  public static void main(String[] args) {
    try {
      ...
      WsGetMembershipsResults results = Grouper.getMemberships(config, groupName);

      String resultWarnings = results.getResponseMetadata().getResultWarnings();
      if (!resultWarnings.isEmpty()) {
        System.out.println("resultWarnings::"+resultWarnings);
      }
  ...
  private static Options getOptions() {
    ...
    Option groupNameOption = Option.builder("g")
        ...
        .argName("group name")
        .required()
        .desc("The Grouper group to list the members of")
        .build();
  ...
  private static void printHelp(Options options) {
    ...
    String header = "Lists the members of a Grouper Group";
    formatter.printHelp(120, "grouper-list", header, options, "", true);
  }
}
